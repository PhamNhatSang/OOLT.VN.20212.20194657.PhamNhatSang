...
public Media getALuckyItem(ArrayList<Media> luckyitem) {
	int index=0;
	do {
 double i=(Math.random()) * ((luckyitem.size() - 0)) + 0;
 index =(int )i;
	}while((this.totalCost()<250&&luckyitem.get(index).getCost()>50)||(this.totalCost()>=250&&luckyitem.get(index).getCost()>100));
	addMedia(luckyitem.get(index));
	luckyitem.get(index).setCost(0);
 return luckyitem.get(index);
}
