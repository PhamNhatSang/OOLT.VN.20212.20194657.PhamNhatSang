...
public void Printlist() {
	...
	for(int i=0;i<this.qtyOrdered;i++) {
		if(this.itemsOrdered[i].getCost()==0) {
		System.out.println(i+"."+"DVD"+"-"+"["+this.itemsOrdered[i].getTitle()+"]"+"-"+"["+this.itemsOrdered[i].getCategory()+"]"+"-"+"["+this.itemsOrdered[i].getDirector()+"]"+"-"+"["+this.itemsOrdered[i].getLength()+"]"+":"+"["+this.itemsOrdered[i].getCost()+"]"+"-This is free item");
...
public DigitalVideoDisc getALuckyItem() {
 ...
 double i=(Math.random()) * ((this.qtyOrdered - 0)) + 0;
 int index =(int ) i;
 for(int j=0;j<this.qtyOrdered;j++) {
	 if(j==index) {
		 this.itemsOrdered[j].setCost(0);
		 return this.itemsOrdered[j];
	 }
 }
 return dvd;
}
